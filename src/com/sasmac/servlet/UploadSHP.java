...
import com.sasmac.gdal.GDALutil;
import com.sasmac.util.ZipUtil;
 ...
 * 
 */
public class UploadSHP extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			List<String> unzipfiles = ZipUtil.unZipFiles(uploadFile, PATH_FOLDER + "\\");
			if (uploadFile.exists()) {
				// uploadFile.delete();
			}
			String shpWKT = "";
			for (int i = 0; i < unzipfiles.size(); i++) {
				String unzipfile = unzipfiles.get(i);
				String fileext = unzipfile.substring(unzipfile.lastIndexOf("."));
				if (fileext.equalsIgnoreCase(".shp")) {
					System.out.println(unzipfile);
					GDALutil gdaLutil = new GDALutil();
					shpWKT = gdaLutil.ShpToWkt(unzipfile);
					break;
				}
			}
