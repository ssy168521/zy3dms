...
public class DataSouPool {
	...
	/**
	 * 用一个链表进行数据库连接的容器， 因为在操作的时候我们使用数据库的连接和它的释放是一个频繁的插入和删除的操作，
	 * 所以在这里最好的最大就是使用链表进行操作
	 */
	private LinkedList<DataSource> pDataSourePool = new LinkedList<DataSource>();

	// private static String connstring =
	// "MySQL:testdb,user=root,port=3306,password=123456,host=localhost";
	...
	/**
	 * 类在构造的时候按照给定的连接池的大小进行数据源的创建操作， 连接创建以后，把创建的对象放置到list中去
	 */
	public DataSouPool() {
	...
	public static DataSource createDataSource() {
		...
		try {
			...
			String connstring = "MySQL:" + database + ",user=" + user + ",port=" + port + ",password=" + password
					+ ",host=" + host;
			// connstring =
			// "MySQL:testdb,user=root,port=3306,password=123456,host=localhost";
