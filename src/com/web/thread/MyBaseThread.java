...
public class MyBaseThread extends Thread {
	...
	public static boolean readParamters() {
		try {
			...
			// szPath = "/msp";
			// }
			// Constants.dataSourcePath = szPath +
			// "/conf/webapp/dss-dms-dataSource.xml";
			// Document doc = xmlParser.parse(szPath +
			// "/conf/webapp/dss-dms-server.xml");
			//
			// Element root = doc.getDocumentElement();
			//
			// //系统日志路径
			// Constants.szLog =
			// root.getElementsByTagName("WriteLog").item(0).getFirstChild().getNodeValue();
			// //存储区文件存放路径
			// Constants.szOnlineDir =
			// root.getElementsByTagName("OnlineDir").item(0).getFirstChild().getNodeValue();
			// //MSP存储位置名称
