...
public class MyArchiveThread extends MyBaseThread implements Runnable {
	...
	public void run() {
		...
		while (true) {
			try {
				 ...
				 * Constants.THREAD_NO_ACTIVE) {//当前卫星归档线程是否启用 Constants.
				 * WriteLog("*********  System config has bean deleted ****************"
				 * ); break; }
				 */
	 ...
	 * 判断文件是否打包完整
	 * <p>
	 * 判断方法:解析xml文件中的filename信息,判定相同路径下所有的filename是否存在
	 * </p>
	 */
	...
	private void getMetaDataFromXML(java.io.File xmlFile) throws Exception {
		...
		// int ScenePath =
		// Integer.parseInt(doc.selectSingleNode("//sensor_corrected_metadata/productInfo/ScenePath").getText());
		// int SceneRow =
		// Integer.parseInt(doc.selectSingleNode("//sensor_corrected_metadata/productInfo/SceneRow").getText());

		// int CloudPercent =
		// Integer.parseInt(doc.selectSingleNode("//sensor_corrected_metadata/productInfo/CloudPercent").getText());
		// int ProductQuality = 0;
		// node =
		// doc.selectSingleNode("//sensor_corrected_metadata/ProductQualityInfo/RadiometricCheck/ProductQuality");
		// if(node == null)
		// node =
		// doc.selectSingleNode("//sensor_corrected_metadata/productInfo/ProductQuality");
		// if(node != null)
		// ProductQuality = Integer.parseInt(node.getText());

		// System.out.println("productId = " + productId);
		// java.util.List<org.dom4j.Node> list =
		// doc.selectNodes("sensor_corrected_metadata//Filename");
	...
	public static void main(String args[]) {
		...
		try {
			// Constants.setThreadStatus("zy301",
			// Constants.THREAD_STATUS_RUNNING);
			new Thread(m).start();
			// m.ergodicDir(new java.io.File("E:\\SRC"));//.isFinishCompress(new
			// java.io.File("E:\\SRC\\zy301a_bwd_026084_026105_20160919112839_01_sec_0001_1609208009.tar"));
			// new Thread(m).start();
			// m.getMetaDataFromXML(new
			// java.io.File("E:\\工作\\测绘寿\史绍雨\\ZY3_01a_mynnavp_890153_20120725_110058_0007_SASMAC_CHN_sec_rel_001_1208063670.xml"));
			// m.getMetaDataFromXML(new
			// java.io.File("E:\\工作\\测绘寿\史绍雨\\zy302a_nad_000944_006157_20160731111715_01_sec_0001_1609284788.xml"));
			// org.dom4j.io.SAXReader reader = new org.dom4j.io.SAXReader();
			// org.dom4j.Document doc = reader.read(new
			// java.io.File("E:\\工作\\测绘寿\史绍雨\\ZY3_01a_mynnavp_890153_20120725_110058_0007_SASMAC_CHN_sec_rel_001_1208063670.xml"));
			// java.util.List<org.dom4j.Node> list =
			// doc.selectNodes("sensor_corrected_metadata//path",
			// "sensor_corrected_metadata//filename");
