...
public class WebDaoImpl implements WebDao {
	...
	@Override
	public boolean addSysConfig(Connection conn, ConfigureForm item) throws Exception {
		String sql = "insert into SysConfig(satellite, archivePath, destPath, storageRule, checkRule, archiveStart, archiveEnd)"
				+ " values(?,?,?,?,?,?,?)";

		Object[] params = { item.getSatellite(), item.getArchivePath(), item.getDestPath(), item.getStorageRule(),
				item.getCheckRule(), item.getArchiveStart(), item.getArchiveEnd() };

		return new DataUtils(conn).update(sql, params) > 0 ? true : false;
	}

	@Override
	public List<ConfigureForm> getAllSysConfig(Connection conn) throws Exception {
		...
		ResultSetHandler<List<ConfigureForm>> handler = new BeanListHandler<ConfigureForm>(ConfigureForm.class);
		return new DataUtils(conn).search(sql, handler, null);
	}

	@Override
	public boolean isFileArchive(Connection conn, String tablename, String fileName) throws Exception {
		String sql = "select count(1) as mycount from " + tablename + " where filename = ?";
		Object[] params = { fileName };
		return new DataUtils(conn).searchForInt(sql, params, "mycount") > 0 ? true : false;
	}

	@Override
	public boolean insertProduct(Connection conn, DataInfo dataInfo) throws Exception {
		...
		Object[] params = { dataInfo.getArchiveFile(), dataInfo.getStorageFile(), dataInfo.getDataId(),
				dataInfo.getOrbitID(), dataInfo.getScenePath(), dataInfo.getSceneRow(), dataInfo.getSatellite(),
				dataInfo.getSensor(), dataInfo.getAcquisitionTime(), dataInfo.getProductLevel(),
				dataInfo.getCloudPercent(), dataInfo.getProductQuality(), dataInfo.getFileSize() };

		return new DataUtils(conn).update(sql, params) > 0 ? true : false;
	}

	@Override
	public List<DataInfo> getDataInfo(Connection conn, DataInfo data) throws Exception {
		...
		ResultSetHandler<List<DataInfo>> handler = new BeanListHandler<DataInfo>(DataInfo.class);
		return new DataUtils(conn).search(sql, handler, null);
	}
	...
	@Override
	public boolean updateSysConfig(Connection conn, ConfigureForm item) throws Exception {
	...
	@Override
	public boolean deleteSysConfig(Connection conn, ConfigureForm item) throws Exception {
	...
	@Override
	public boolean isFileArchive(Connection conn, String fileName, long fileSize) throws Exception {
		...
		if (list.size() > 0) {
			return Integer.parseInt(list.get(0).get("mycount").toString()) > 0 ? true : false;
		}
		return false;
	}

	@Override
	public ConfigureForm getConfigById(Connection conn, int id) throws Exception {
	...
	@Override
	public void updateStaticts(Connection conn, String satellite, int status) throws Exception {
		String sql = "";
		if (status == 1) {
	...
	@Override
	public List<Staticstis> getStaticstisInfo(Connection conn, Staticstis s) throws Exception {
		String sql = "select id,satellite,sensor,sucess_count,failed_count from statistics";

		ResultSetHandler<List<Staticstis>> handler = new BeanListHandler<Staticstis>(Staticstis.class);
		return new DataUtils(conn).search(sql, handler, null);
	}

	@Override
	public int addTask(Connection conn, String destPath, int... ids) throws Exception {
		...
		if (new DataUtils(conn).update(sql, params) > 0) {
			...
			for (int id : ids) {
				sql = "insert into taskData(taskId, dataId) values(" + taskId + "," + id + ")";
	...
	public List<TaskInfo> getTask(Connection conn) throws Exception {
		...
		ResultSetHandler<List<TaskInfo>> handler = new BeanListHandler<TaskInfo>(TaskInfo.class);
		return new DataUtils(conn).search(sql, handler, null);
	}

	@Override
	public List<TaskData> getTaskData(Connection con, int taskId) throws Exception {
		return null;
	}

	@Override
	public void updataTaskStatus(Connection conn, int taskId, String status) throws Exception {
