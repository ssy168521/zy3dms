...
public class renamefile {
	...
	/*
	 * public static String decode(String bytes) { ByteArrayOutputStream baos =
	 * new ByteArrayOutputStream(bytes.length() / 2); //
	 * 灏嗘瘡2浣�6杩涘埗鏁存暟缁勮鎴愪竴涓瓧鑺� for (int i = 0; i < bytes.length(); i += 2)
	 * baos.write((hexString.indexOf(bytes.charAt(i)) << 4 | hexString
	 * .indexOf(bytes.charAt(i + 1)))); return new String(baos.toByteArray()); }
	 */
}
